<template>
  <v-container fluid>
    ...
    <v-row v-if="hasCollectionItems">
      <CardCol v-for="item in collectionOrFilteredCollection" :key="item.guid">
        <v-lazy :options="{ threshold: 0.5 }" transition="fade-transition">
          <YoutubeCard :item="item" :guid="item.guid" :title="item.title" :mediaId="item.media_id" :videoId="item.index"
            :spotifyId="item.spotify_id" :thumbnail="item.thumbnail" :collected="item.collected"
            :key="searchQueryUpdates"></YoutubeCard>
        </v-lazy>
      </CardCol>
    </v-row>
    ...
    <v-row v-if="!hasCollectionItems && !showLoadingBar" class="justify-center mb-6">
    ...
    <v-row v-if="!hasCollectionItems && !showLoadingBar" class="justify-center">
      <img class="rounded mt-6" src="https://media.giphy.com/media/6uGhT1O4sxpi8/giphy.gif" width="480" height="240" />
    </v-row>
  </v-container>
</template>
